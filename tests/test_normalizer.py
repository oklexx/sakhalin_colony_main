...
def test_normalizer_load_rejects_obs_size_mismatch(tmp_path):
    """A v1 (289) file on a v0 (248) normalizer must fail, not misalign."""
    from cpp_env import Normalizer

    norm = Normalizer(obs_size=289)
    norm._rms.set_mean([0.0] * 289)
    norm._rms.set_var([1.0] * 289)
    path = tmp_path / "norm.json"
    norm.save(str(path))

    with pytest.raises(ValueError) as excinfo:
        Normalizer(obs_size=248).load(str(path))
    assert "normalization mismatch" in str(excinfo.value)
    assert "289" in str(excinfo.value)
    # matching size still loads
    Normalizer(obs_size=289).load(str(path))
