...
## [2026-09-21] — штраф за перемотку времени (DAY/WEEK): новые параметры наград + индикатор в мониторинге

Проблема: DAY/WEEK — основные действия модели. Причина в экономике наград:
DAY даёт +0.3…1.5/шаг «ни за что» (`tax_daily_bonus` + `chain_daily` +
`extraction_daily`), а WEEK собирает те же слагаемые ×7 дней В ОДИН шаг
(`chain_daily` 0.5 → 3.5, `extraction_daily` 0.3 → 2.1) плюс сжимает горизонт
для `goal_survival_coeff` (10k шагов = 70k дней). `idle_build_penalty` (−2 за
7 дней) это не перевешивает, а BUILD несёт риск `error_penalty` −2 — поэтому
«спать» оптимально. Легитимное ожидание (стройка идёт, денег нет) при этом
ломать нельзя — нужен именно настраиваемый штраф, а не запрет.

### Новые поля RewardConfig: `day_penalty`, `week_penalty` (дефолт 0.0)

- `include/colony/reward_config.h`, `rl/config.py`, `src/bindings.cpp`
  (`def_readwrite`), `configs/reward_v4.json` + `configs/reward_v3.json` (0.0 =
  старое поведение, «чистый v3» сохранён), `src/gui.cpp` (парсинг из профиля —
  иначе окно наблюдения считало бы без штрафа), таблица `kFields` в
  `tests/cpp/reward_v4_longrun.cpp` (строгая к неизвестным ключам).
- `src/env.cpp`: `rew -= day_penalty` при `A_DAY`, `rew -= week_penalty` при
  `A_WEEK` (фикс за нажатие, не за день — ранний обрыв недели по событию скидки
  не даёт); BUILD и менеджеры тоже двигают календарь, но штрафа не несут.
  Новый компонент `time=` в step-логе — сумма компонентов по-прежнему сходится.
- Синхронизация по золотому правилу: регрессы `test_reward_field_sync.py`
  (C++ ↔ Python ↔ биндинги ↔ UI) и `test_reward_default_profile.py`
  (дефолты = профиль) покрывают новые поля без изменений.
- Ориентир подбора: `day_penalty` 0.2–0.5 (съедает «спокойный день»),
  `week_penalty` 2.0–4.0 (на уровне `error_penalty`; выше −2 осторожно — иначе
  станет выгоден спам ошибочных BUILD). Альтернатива без новых полей: усилить
  «Штраф простоя» / снизить «Порог простоя», урезать `chain_daily`,
  `extraction_daily`, `tax_daily_bonus` — но это бьёт и по строящей политике.

### UI

- Вкладка «Награды»: новая группа «Перемотка времени» (`day_penalty`,
  `week_penalty`) с тултипами и строками глоссария (`HUMAN_REWARD_HELP`;
  регресс `test_stage1_preset.py` требует описание каждого ключа).
- Вкладка «Мониторинг»: индикатор «перемотка: N% (день X% + неделя Y%)» —
  зелёный <30%, жёлтый 30–60%, красный ≥60% («модель спит» + подсказка, какие
  штрафы крутить). Считается из `top_actions` последних ~1000 шагов.
