...
class ActionLegalityMonitor:
    """Доля env-шагов, на которых действие было ДОСТУПНО (маска == 1).

    Это доступность, а не «легальность выбранного действия»: выбрать закрытое
    действие политика не может вовсе, поэтому доля легальных выборов всегда
    100% и ничего не говорит. Имя класса и поле `action_legality` остались
    историческими (совместимость протокола), в UI подпись — «доступно, %»
    (P3-7 ревью 2026-09-24).
