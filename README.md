...
**Версия obs** (`obs_version`, дефолт 1): v1 дописывает в конец наблюдения «рамку» — 9 эффективных весов ресурсов (при `all_resources` это девять 1.0, ровно то, что применяет экономика) + 32 бита `build_allowed` (287 = 246+9+32); v0 — строгий префикс, `counts`/`idle_by_type` не фильтруются никогда. Версия фиксируется при создании среды (`--obs-version 0/1` в `train.py` / `watch_champion.py` / `run_eval`), смена на ходу запрещена (`set_curriculum` бросает). Версия хранится в чекпойнте, `meta.json` и `.norm.json`; несовпадение чекпойнт↔среда — явная ошибка «obs mismatch» с подсказкой нужного `--obs-version`, а не падение в matmul; старый `.norm.json` на 246 чисел v1-среда (287) отклоняет. Опциональная абляция `obs_mask_locked_catalog` (reward JSON, дефолт `false`) зануляет строки каталога закрытых зданий; по умолчанию каталог гейт игнорирует.
